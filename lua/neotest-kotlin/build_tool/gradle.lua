...
local binaries = require("neotest-kotlin.command.binaries")
...
local function to_gradle_path(dependency)
  ...
  local dir =
    string.format("%s/.gradle/caches/modules-2/files-2.1/%s/%s/%s", os.getenv("HOME"), group_id, artifact_id, version)
...
gradle.get_dependencies_classpath = function()
  ...
  local suc =
    os.execute(binaries.gradle() .. " dependencies > build/neotest-kotlin" .. "/dependencies.txt " .. "< /dev/null")
